...
const valueLabels = document.querySelector("#valueLabels");
const radialBars = document.querySelector("#radialBars");
...
function createLabels(parent, radius, values) {
    const angleStep = 360 / values.length;
    values.forEach((value, i) => {
        const angle = (i + 1) * angleStep;
        const labelGroup = document.createElementNS(svgNS, "g");
        labelGroup.setAttribute("transform", `rotate(${angle}) translate(0,${radius}) rotate(${-angle})`);
...
function createGradients(parent, rot, num) {
...
function createRadialBars(parent, rot, rInner, rOuter, values) {
